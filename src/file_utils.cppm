...
export namespace FileUtils {
    ...
    void traverse_directory(const std::filesystem::path& dir_path, ThreadSafeQueue& file_queue, 
        std::atomic<size_t>& total_files, int max_depth = 10);
}

export namespace FileUtils {
    bool is_source_file(const std::filesystem::path& path) { // Check if file is a source file based on extension
    ...
    void traverse_directory(const std::filesystem::path& dir_path, ThreadSafeQueue& file_queue, 
        ...
        std::function<void(const std::filesystem::path&, int)> traverse = [&](const std::filesystem::path& path, int depth) {
            if (depth > max_depth) return; // Base case for recursion depth

            for (const auto& entry : std::filesystem::directory_iterator(path)) { // Iterate directory contents
                const auto& current_path = entry.path(); // Get current entry path

                if (std::filesystem::is_directory(current_path)) { // Recurse into subdirectories
                    traverse(current_path, depth + 1);
                // Process source files
                } else if (std::filesystem::is_regular_file(current_path) && is_source_file(current_path)) {  
